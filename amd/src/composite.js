...
define([
  ...
  "core/config",
  "core/notification",
], function ($, Str, Participants, ajax, config, notification) {
  /**
  ...
  function membersPerGroupChanged() {
    ...
    Str.get_strings(stringkeys).then(function (patterns) {
      ...
      var groupsAtMost = calculateGroupsDistributionAtMost(
        membersSize,
        membersPerGroup
      );
      if (groups.length == 0 && groupsAtMost.length > 0) {
        $("#composite__members_per_group_help").text(
          localizeDistribution(groupsAtMost, patterns)
        );
        $("#composite__at_most").val("true");
      } else if (
        (groups.length > 0 && groupsAtMost.length == 0) ||
        groups.toString() == groupsAtMost.toString()
      ) {
        $("#composite__members_per_group_help").text(
          localizeDistribution(groups, patterns)
        );
        $("#composite__at_most").val("false");
      } else {
        $("[for=composite__members_per_group_at_most_false]").text(
          localizeDistribution(groups, patterns)
        );
        $("[for=composite__members_per_group_at_most_true]").text(
          localizeDistribution(groupsAtMost, patterns)
        );
        $("#composite__at_most").val("true");
  ...
  function localizeDistribution(distribution, patterns) {
    if (distribution.length == 2) {
      return patterns[0]
        .replace(/\{\{groups\}\}/g, distribution[0])
        .replace(/\{\{size\}\}/g, distribution[1]);
    } else if (distribution.length == 4) {
      return patterns[1]
        .replace(/\{\{groups1\}\}/g, distribution[0])
        .replace(/\{\{size1\}\}/g, distribution[1])
        .replace(/\{\{groups2\}\}/g, distribution[2])
        .replace(/\{\{size2}}/g, distribution[3]);
    } else {
      return "";
    }
  }
  ...
  function requirementsAddClicked(event) {
    ...
    var requirementElement = $(
      '<li class="list-group-item" data-factor="' + factor + '"></li>'
    );
    var values = {
      factor: $("#composite__requirements_factor_" + factor)
        .text()
        .trim()
        .toLowerCase(),
      level: $("#composite__requirements_level_" + level)
        .text()
        .trim()
        .toLowerCase(),
      importance: $("#composite__requirements_importance_" + importance)
        .text()
        .trim()
        .toLowerCase(),
    };
    Str.get_string("composite_requirements_pattern", "block_tog", values).then(
      function (pattern) {
  ...
  return {
    ...
    initialise: function ($params) {
      $("#composite__selected_role_for_users").change(
      ...
      $("#composite__members_per_group_at_most_true").click(
        membersPerGroupAtMostTrueClicked
      );
      $("#composite__members_per_group_at_most_false").click(
        membersPerGroupAtMostFalseClicked
      );
